...
void send_file_response(struct hitArgs *args, char *path, char *request_body, int path_length)
{
    ...
    else if (path_ends_with(path, "jquery-3-2-1-min.js"))
    {
        string_add(response, "text/javascript");
        write_header(args->socketfd, string_chars(response), jquery_3_2_1_min_js_len);
        write(args->socketfd, jquery_3_2_1_min_js, jquery_3_2_1_min_js_len);
    }
    else
    {
